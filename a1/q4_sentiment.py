...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    # words = wordVectors[tokens[sentence]] # Why isn't this parallelizable?
    words = wordVectors[[tokens[sent] for sent in sentence]]
    sentVector = np.average(words, axis = 0)
    ### END YOUR CODE
...
def outputPredictions(dataset, features, labels, clf, filename):
    ...
    with open(filename, "wb") as f:
        f.write(b"True\tPredicted\tText\n")
        for i in range(len(dataset)):
            f.write (b"%d\t%d\t%s\n" % (labels[i], pred[i], b" ".join(dataset[i][0])))
