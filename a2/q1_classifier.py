...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        word_dim = self.config.batch_size
        hidden_dim = self.config.n_features
        num_class = self.config.n_classes
        self.input_placeholder = tf.placeholder(
            tf.float32, shape=[word_dim, hidden_dim], name='input_placeholder')
        self.labels_placeholder = tf.placeholder(
            tf.float32, shape=[word_dim, num_class], name='labels_placeholder')
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        hidden_dim = self.config.n_features
        num_class = self.config.n_classes
        self.W = tf.get_variable('q1_classifier_W', shape=[hidden_dim, num_class],   dtype=tf.float32,
                                 initializer=tf.random_uniform_initializer(-np.sqrt(1./hidden_dim), np.sqrt(1./hidden_dim)))
        self.b = tf.get_variable('q1_classifier_b', shape=[num_class],   dtype=tf.float32,
