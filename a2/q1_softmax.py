...
def softmax(x):
    ...
    xx = tf.exp(x - tf.reshape(x_max, (-1,1)))
    out = xx / tf.reshape(tf.reduce_sum(xx, reduction_indices=[1]), (-1,1))
    ### END YOUR CODE
...
def test_cross_entropy_loss_basic():
    ...
    test1 = cross_entropy_loss(tf.constant(
        y, dtype=tf.float32), tf.constant(yhat, dtype=tf.float32))
    with tf.Session() as sess:
        test1 = sess.run(test1)
    expected = -3 * np.log(.5)
