...
class Config:
    ...
    ### YOUR CODE HERE
    # 0 # The total number of features used for each window.
    n_window_features = (window_size * 2 + 1) * n_word_features
    ### END YOUR CODE
    n_classes = 5
    dropout = 0.5
    embed_size = 50
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    windowed_data = []
    for _sentence, labels in data:
		### YOUR CODE HERE (5-20 lines)
        sentence = _sentence.copy() # make a copy
        sentence_len = len(sentence)
        sentence[0:0] = [start] * window_size
        sentence.extend([end] * window_size)
        for i in range(sentence_len):
            cur = sum(sentence[i : i + 2*window_size + 1], [])
            windowed_data.append((cur, labels[i]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.int32, shape=[None, self.config.n_window_features], name='WindowModel_input_placeholder')
        self.labels_placeholder = tf.placeholder(tf.int32, shape=[None], name='WindowModel_labels_placeholder')
        self.dropout_placeholder = tf.placeholder(tf.float32, shape=(), name='WindowModel_dropout_placeholder')
        ### END YOUR CODE
